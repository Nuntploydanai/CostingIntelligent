...
# Reload the module
import importlib
import fabrication_calc_v2
importlib.reload(fabrication_calc_v2)
from fabrication_calc_v2 import compute_fabric_row

# Test with Excel values including price override
result = compute_fabric_row({
    ...
    'price_value': 2.63,  # Excel has this override
})

print("Fabrication result with price override 2.63:")
...
print(f"  PRICE / Lbs (used): {result['price_lb']}")
...
if isinstance(result['default_price_yd'], (int, float)):
    diff_yd = abs(result['default_price_yd'] - 1.319)
    print(f"\nPrice/YD difference: {diff_yd:.6f}")
